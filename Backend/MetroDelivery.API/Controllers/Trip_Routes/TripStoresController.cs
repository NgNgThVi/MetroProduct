...
using MetroDelivery.Application.Features.Trip_Routes.Queries.GetAllTrip_Route;
...
namespace MetroDelivery.API.Controllers.Trip_Routes
{
    ...
    public class TripStoresController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("get-all-trip-route")]
        public async Task<List<TripRouteResponse>> GetAll()
        {
            var response = await _mediator.Send(new GetListTripRouteQuery());
            return response;
        }
